...
class MoviesAPI:
    ...
    def __init__(self, amount_of_pages):
        ...
        self.__movie_api_url = "https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&sort_by=popularity.desc&page={!!!!}"
        self.__genre_api_url = "https://api.themoviedb.org/3/genre/movie/list?language=en"
        self.__themoviedb__headers = {
    ...
    def __movie_request_handler(self, amount_of_pages):
        for i in range(amount_of_pages):
            response = requests.get(f"{self.__movie_api_url}&page={i+1}", headers=self.__themoviedb__headers)
    ...
    def get_movies_with_keywords(self, keywords):
        return list(filter(lambda movie: any(keyword.lower() in movie["overview"].lower() for keyword in keywords), self.get_all_movies()))



    def get_all_genre(self):
        return requests.get(f"{self.__genre_api_url}", headers=self.__themoviedb__headers).json()["genres"]
    ...
    def get_structured_films(self):
        ...
        for movie in self.get_all_movies():
            ...
            list_of_movies.append(Movie(Title=movie["title"], Popularity=movie["popularity"], Score=int(movie["vote_average"]), Last_day_in_cinema=last_day_in_cinema.strftime("%Y-%m-%d")))
